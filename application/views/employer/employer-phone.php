...
<!-- 
	...
	<div class="block-phone">
		<div class="phone-cont">
			<p class="phone-cont__title">Вакансий для вас 247</p>
			<p class="phone-cont__text">Найдите работу прямо сейчас</p>
			<p class="phone-cont__text">Укажите ваш мобильный номер, мы отправим сообщение с кодом подтверждения. Так мы будем знать, что телефон принадлежит именно вам.</p>


			<form action="/register/step/employer/phone" method="post" class="form-phone">
				<div class="block-input">
					<p class="block-input__text">Номер телефона</p>
					<input name="phone" type="tel" placeholder="+7 XXX XXX-XX-XX">
					<?php if(isset($error))echo '<p style="font-size: 10px; color: red;">'.$error.'</p>';?>
				</div>
				<button>Продолжить</button>
			</form>
			<p class="phone-cont__quest">Уже есть аккаунт? <a href="login.html">Войти</a></p>
		</div>



		<p class="block-phone__text">Сервис «My Best Helper» собирает персональные данные для создания вашего аккаунта. Это позволит вам взаимодействовать с другими участниками сообщества. При необходимости вы всегда можете разорвать отношения с нашим сервисом.</p>
	</div>
	...
	<div class="block-bg" id="popapTwo">
		...
		<form action="/register/change_lang" method="post" id="change_lang_form_ru">
			<input type="hidden" name="type" value="ru">
		</form>
		<form action="/register/change_lang" method="post" id="change_lang_form_en">
			<input type="hidden" name="type" value="en">
		</form>
		<script>
			$('.header-cont__en').click(function(){
				$('#change_lang_form_en').submit();
				return false;
			});
			$('.header-cont__ru').click(function(){
				$('#change_lang_form_ru').submit();
				return false;
			});
		</script>
	</body>
